...
// Computing Cholesky factorization of A and store the matrix L in the lower part of A.
void chol(std::vector < std::vector<double>>&A)
{
  for (int i = 0; i<A.size(); i++)
    {
      for (int j = 0; j < i; j++)
...
// Solve the low-triangle matrix linear equation: A x = b, to get the solution vector x and
// store it in vector b;
void low_solve(std::vector<std::vector<double>> A, std::vector<double> &b)
{
  ...
  for(int i=1;i<b.size();i++)
      ...
      for (int j=0;j<i;j++)
	{
      ...
      b[i]=(b[i]-tempsum)/A[i][i];
    }
}

// compute the inverse of A, return it in A in fact it is stored as A*.
void low_inverse(std::vector<std::vector<double>> &A)
{
  std::vector<std::vector<double>> tempA(A);
  int n=A.size();
  std::vector<double> ei(n,0);
  for (int i=0;i<n;i++)
    {
      ei[i]=1;
      low_solve(tempA,ei);
